...
exports = module.exports = functions.auth.user().onCreate((uRecord, context) => {
  ...
  return admin.auth().getUser(userRecord.uid).then(user => {
    ...
    if (email) {
      const mailOptions = {
        from: `"joão Pedro Massari" <${gmailEmail}>`,
        to: email,
        subject: `Portal de relatorios`,
        text: `${displayName || ''},Bem vindo ao novo portal do Ministério Público. Esperamos que tenha uma ótima experiencia na nossa aplicação.`
      }
